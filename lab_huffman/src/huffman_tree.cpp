...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* stillSmall = removeSmallest(singleQueue, mergeQueue);
        TreeNode* combo = new TreeNode(smaller->freq.getFrequency() + stillSmall->freq.getFrequency());
        combo->left = smaller;
        combo->right = stillSmall;
        mergeQueue.push(combo);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()) {
        bool nextB = bfile.getNextBit();
        if(nextB) {
            Frequency friq(bfile.getNextByte(), 0);
            TreeNode* temp = new TreeNode(friq);
            return temp;
        }
        else{
